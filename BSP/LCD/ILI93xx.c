 ...
 * MACRO
 ********************************************************************************/
#define READ_CHIP_MODEL_CMD			0xD3 // 获取 LCD 型号
#define GRAM_PTR_MOVE_DIRECTION		0x36 // 获取 GRAM(Graphical RAM) 现存存储器访问指针的增长方向, 进而修改显存, eg:从左至右, 从上至下
#define SET_X_POS					0x2A // 设置 x 坐标, 0 <= x <= 239
#define SET_Y_POS					0x2B // 设置 y 坐标, 0 <= y <= 319
#define WRITE_GRAM					0x2C // 写显存 GRAM
#define READ_GRAM					0x2E // 读显存 GRAM

// NOTE:
// 另外，特别注意 ILI9341 所有的指令都是 8 位的（高 8 位无效），且参数
...
/**
 * @brief 写数据函数,可以替代LCD_WR_DATAX宏,拿时间换空间.
 * 
 * @param data 寄存器值, 16 bit 对应 PB[0:15]
 */
...
u16 LCD_RD_DATA(void)
{
    ...
    //读取数据(读寄存器时,并不需要读2次)
    LCD_RD_CLR;
    if (lcddev.id == 0X8989) // 型号 0x8989 需要延时 2us ?
...
{	 
    switch (lcddev.id) {
        ...
        case 0X5310:
        {
            ...
            LCD_WR_DATA(Xpos >> 8); // 先写高字节
            LCD_WR_DATA(Xpos & 0XFF); // low
        ...
        case 0X6804:
        {
            if(lcddev.dir == LCD_HORIZONTAL_SCREEN)
...
/**
 * @brief 设置 LCD 显示方向, 横屏其实就是调转 x, y 坐标
 * 
 * @param dir dir:0,竖屏；1,横屏 
 */
void LCD_Set_Display_Dir(u8 dir)
{
    if(dir == LCD_VERTICAL_SCREEN) //竖屏
    {
        lcddev.dir = LCD_VERTICAL_SCREEN;	//竖屏
        lcddev.width = 240; // X
        lcddev.height = 320; // Y
...
void LCD_Set_Window(u16 sx,u16 sy,u16 width,u16 height)
{
    ...
    {
        if(lcddev.dir == LCD_HORIZONTAL_SCREEN)//横屏
        {
            //窗口值
            hsaval=sy;
            heaval=theight;
 ...
 *        本函数占用较大 flash, 用户可以根据自己的实际情况,删掉未用到的LCD初始化代码.以节省空间.
 *        初始化时序按照 datasheet 即可
 */
void LCD_Init(void)
{ 
 ...
 * @brief 清屏函数
 * 
 * @param color 要清屏的填充色, bit depth:2bytes, 每一个像素点都是 2bytes,
 */
void LCD_Clear(u16 color)
{
    u32 index=0;      
    u32 totalpoint = lcddev.width * lcddev.height; //得到总像素点数

    if ((lcddev.id == 0X6804) && (lcddev.dir == LCD_HORIZONTAL_SCREEN))//6804横屏的时候特殊处理  
    {
        lcddev.dir = LCD_VERTICAL_SCREEN;	 
        lcddev.setxcmd = 0X2A;
        lcddev.setycmd = 0X2B;  	 			
...
void LCD_Fill(u16 sx,u16 sy,u16 ex,u16 ey,u16 color)
{
    ...
    if ((lcddev.id==0X6804) && (lcddev.dir== LCD_HORIZONTAL_SCREEN))	//6804横屏的时候特殊处理  
    {
        ...
        lcddev.dir = LCD_HORIZONTAL_SCREEN;	 
        lcddev.setxcmd = 0X2B;
        lcddev.setycmd = 0X2A;  	 
    } else {
        xlen = ex - sx + 1;	 
...
void LCD_ShowChar(u16 x, u16 y, u8 character, u8 size, u8 mode)
{
    ...
    for(t = 0; t < csize; t++) {
        if(size == 12)
            ...
            temp = asc2_1608[character][t]; //调用1608字体
        else if(size == 24)
 ...
 * @param x x,y:起点坐标
 * @param y x,y:起点坐标
 * @param width 要显示字符串的区域宽度
 * @param height 要显示字符串的区域高度
 * @param size 字符的大小:12/16/24
 * @param p 字符串
 * @return * void 
 */
