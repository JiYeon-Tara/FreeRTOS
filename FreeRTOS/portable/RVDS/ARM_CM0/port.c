...
//4 个 8 位的寄存器组成一个 32 位的寄存器
//每一个中断对应一个 8 位的优先级寄存器
//0xE000ED20 - 0xE000ED23
...
//PendSV 和 SysTick 的优先级分别对应 0xE000_ED20 地址的高 8 位(bit[31:24]) 和 次高 8 位(bit[24:16])
