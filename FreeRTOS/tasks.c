...
typedef struct tskTaskControlBlock
{
	...
	//状态列表项:Ready, Blocked, Suspended; 链表的使用, 定义一个结构体成员, 包含 ListItem_t 成员
	...
	//使用使用静态分配内存的方式创建任务的标志: pdTRUE / pdFALSE
...
//如果使用了宏: configCHECK_FOR_STACK_OVERFLOW, 用户就必须提供一个 钩子函数
//堆栈溢出的钩子函数（回调函数）
	...
	BaseType_t xReturn;
		...
		#if( portSTACK_GROWTH > 0 )
		{
			...
			if( pxNewTCB != NULL )
			{
				...
				//这里分配的内存大小： ( ( size_t ) usStackDepth ) * sizeof( StackType_t ) )
				//#define portSTACK_TYPE	uint32_t
				//typedef portSTACK_TYPE StackType_t;
				//所以实际分配的内存大小是: usStackDepth 的 4 倍
		...
		StackType_t *pxStack;
			...
			/* Allocate space for the stack used by the task being created. OS 分配 任务栈*/
			pxStack = ( StackType_t * ) pvPortMalloc( ( ( ( size_t ) usStackDepth ) * sizeof( StackType_t ) ) ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */

			if( pxStack != NULL )
			{
				/* Allocate space for the TCB. OS 分配 TCB*/
...
UBaseType_t x;
	...
	#if( portSTACK_GROWTH < 0 )
	{
		// 栈顶指向地址最大的位置: pxNewTCB->pxStack  + ( ulStackDepth - ( uint32_t ) 1 )
