...
#if 1
#pragma import(__use_no_semihosting)  // 这是什么写法      
//标准库需要的支持函数                 
struct __FILE 
{ 
	int handle; 
...
{   
	//USART->State Register
	while((USART1->SR & 0X40) == 0) //循环发送,直到发送完毕
		;   
    USART1->DR = (u8) ch; // Data Register  
	return ch;
}
#endif //
...
void USART1_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾), 回车, 换行
	{
		...
		{
			if(USART_RX_STA & 0x4000)//接收到了0x0d bit[14], 下一个应该是 0x0A
			{
				if(Res != 0x0a)
			...
			else //还没收到0X0D
			{
				...
				else
				{
					...
					USART_RX_STA++;
					if(USART_RX_STA > (USART_REC_LEN-1)) //接收数据错误,重新开始接收, 一次最多接收 USART_REC_LEN（一次的意思是到下一个/r/n）
						USART_RX_STA = 0;
				}		 
			}
		}   		 
