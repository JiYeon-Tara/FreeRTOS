...
#define UART1_PRIEMPTION_PRIORITY  NVIC_PREEMPTION_PRIORITY_MIDDLE
#define UART1_SUB_PRIORITY         NVIC_SUBPRIORITY_MIDDLE
#define UART1_NVIC_GROUP           DEFAULT_NVIC_GROUP	

#define UART_DEBUG_MAX_BUFF_SIZE			128				// 串口接收 buffer 大小
#define SUPPORT_AT_CMD						1  				// 是否支持 AT 命令


//加入以下代码,支持 printf 函数,而不需要选择use MicroLIB	  
#if 1
...
void uart1_send(const u8 *p_data, u16 len)
{
    ...
    for (i = 0; i < len; ++i) {
        //等待上一次串口数据发送完成  
        while((USART1->SR & 0X40) == 0); // 0100 0000, bit[6], TX complete flag bit
        USART1->DR = (u8) p_data[i]; //写DR,串口1将发送数据
    }
