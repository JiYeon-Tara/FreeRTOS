...
// 中断优先级
#define RTC_PRE_PRIOR			0
#define RTC_SUB_PRIOR			0
#define RTC_NVIC_GROUP			2
...
u8 RTC_Init(void)
{
	//检查是不是第一次配置时钟
	u8 temp=0;
	if(BKP->DR1 != 0X5051)//之前使用的不是LSE, 重新配置 RTC
	{
	  	RCC->APB1ENR |= 1 << 28;     // 使能电源时钟	    
		RCC->APB1ENR |= 1 << 27;     // 使能备份时钟, BKP   
		PWR->CR |= 1 << 8;           // 取消备份区写保护, 允许写
		RCC->BDCR |= 1 << 16;        // 备份区域软复位	   
		RCC->BDCR &= ~(1 << 16);     // 备份区域软复位结束	  	 
	    RCC->BDCR |= 1 << 0;         // 开启外部低速振荡器 
	    while((!(RCC->BDCR & 0X02)) && temp < 250)/* 等待外部时钟 32KHz 就绪 */
		{
			temp++;
			delay_ms(10);
		};
		...
		if(temp>=250)  //return 1; //初始化时钟失败,晶振有问题	   
		{
			...
			BKP->DR1=0X5050; //标记使用LSI作为RTC时钟
		}
		else 
        {
            RCC->BDCR |= 1 << 8; //LSE作为RTC时钟 
            BKP->DR1 = 0X5051; //标记使用LSE作为RTC时钟
        }
		RCC->BDCR |= 1 << 15;//RTC时钟使能	
		
	  	while(!(RTC->CRL & (1 << 5))) // bit[5]
			;//等待RTC寄存器操作完成	 
    	while(!(RTC->CRL & (1 << 3))) // bit[3]
			;//等待RTC寄存器同步  
		...
		RTC->CRL &= ~(1 << 4);           //配置更新， RTC_CRC->bit[4] = 0;
		while(!(RTC->CRL&(1<<5)))
			;   //等待RTC寄存器操作完成		 									  
	 	printf("RTC FIRST SET TIME\n");
	}
